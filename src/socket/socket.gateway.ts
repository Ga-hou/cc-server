...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleConnection(client: SocketInterface) {
    ...
    if (
      !(config as any).turnorigins ||
      (config as any).turnorigins.indexOf(origin) !== -1
    ) {
      config.turnservers.forEach(function(server) {
        const hmac = crypto.createHmac('sha1', server.secret);
        const username =
          Math.floor(new Date().getTime() / 1000) +
          parseInt(String(server.expiry || 86400), 10) +
          '';
  ...
  async sendWelcome(@ConnectedSocket() client: Socket, @MessageBody() data) {
